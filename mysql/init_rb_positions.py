...
class RBPositionInitializer:
    ...
    def verify_initialization(self):
        """验证初始化结果"""
        # 查询总记录数
        self.cursor.execute("SELECT COUNT(*) FROM rb_position_data")
        ...
        print("\n  按 PLC 分组统计:")
        for plc, count in plc_stats:
            print(f"    {plc}: {count} 个位置")
        print("="*50)
...
def main():
    ...
    db_config = {
        ...
        'user': 'root',            # 数据库用户名
        'password': 'root',  # 数据库密码 - 请修改
        'database': 'rollerbed_tracking_db'   # 数据库名称 - 请修改
    }
    ...
    try:
        ...
        # 3. 初始化位置数据
        # 注意: clear_first=True 会先清空现有数据,首次初始化建议设为 False
        success, error = initializer.initialize_positions(
            config, 
            clear_first=False  # 首次初始化设为 False
        )
