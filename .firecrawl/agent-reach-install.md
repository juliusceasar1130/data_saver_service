...
\> agent-reach configure xhs-cookies '\[{"name":"web\_session","value":"xxx","domain":".xiaohongshu.com",...}\]'
>
...
uv sync && uv run python run\_http.py

\# 方式二：直接用 Python 启动
python -c "
from douyin\_mcp\_server.server import mcp
...
> 无需认证即可解析视频信息和获取下载链接。
\> 如需 AI 语音识别提取文案功能，需要配置硅基流动 API Key（\`export API\_KEY="sk-xxx"\`）。
>
...
\- 兼容旧工具名 \`parse\_douyin\_video\_info\` / \`get\_douyin\_download\_link\` / \`extract\_douyin\_text\`
\- 同时新增 \`parse\_social\_post\_info\` / \`extract\_social\_post\_script\`
...
mcporter config add douyin \
 --command /bin/zsh \
 --arg -lc \
 --arg "cd '$PWD' && exec '.venv/bin/python' -m social\_post\_extractor\_mcp" \
 --env ASR\_PROVIDER=bailian \
 --env ASR\_MODEL=paraformer-v2 \
 --env VISION\_PROVIDER=bailian \
 --env VISION\_MODEL=qwen3-vl-flash \
 --env CLEAN\_PROVIDER=bailian \
 --env CLEAN\_MODEL=qwen-flash \
 --env BAILIAN\_API\_KEY=YOUR\_BAILIAN\_API\_KEY
\`\`\`
...
\| Reddit \| \`rdt\` \| \`rdt search "query"\` / \`rdt read POST\_ID\` \|
\| GitHub \| \`gh\` \| \`gh search repos "query"\` \|
\| Web \| \`curl\` + Jina \| \`curl -s "https://r.jina.ai/URL"\` \|
\| Exa Search \| \`mcporter\` \| \`mcporter call 'exa.web\_search\_exa(...)'\` \|
\| 小红书 \| \`mcporter\` \| \`mcporter call 'xiaohongshu.search\_feeds(...)'\` \|
\| 微博 \| \`mcporter\` \| \`mcporter call 'weibo.get\_trendings(limit: 10)'\` \|
\| 小宇宙播客 \| \`transcribe.sh\` \| \`bash ~/.agent-reach/tools/xiaoyuzhou/transcribe.sh \` \|
\| 抖音 \| \`mcporter\` \| \`mcporter call 'douyin.parse\_douyin\_video\_info(...)'\` \|
